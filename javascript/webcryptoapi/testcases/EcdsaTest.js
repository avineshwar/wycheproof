...
function testEcdsaVectors() {
  ...
  for (var i = 0; i < tv["testGroups"].length; i++) {
    ...
    var hashAlg = tg["sha"];
    if (
      SUPPORTED["ecdsa-curve"].indexOf(curveName) == -1 ||
...
Ecdsa.checkNonceCorrectness = function(msg, r, s, d, k, curveName) {
  ...
  assertTrue(
    "Nonce calculation was incorrect",
      ...
      return Ecdsa.exportKey(key.privateKey)
        .then(function(keyData) {
          ...
          for (var i = 0; i < tc.nTests; i++) {
            promises.push(
              Ecdsa.sign(key.privateKey, tc.msg, tc.hashAlg).then(function(
                sig
              ) {
                return HashUtil.digest(tc.hashAlg, tc.msg).then(function(
                  digest
                ) {
...
function testEcdsaBiasAll() {
  ...
  var nTests = 1024;
  var minCount = 410;
  var biasTest256 = new EcdsaBiasTestCase(
    "SHA-256",
    "P-256",
    msg,
    nTests,
  ...
  var biasTest384 = new EcdsaBiasTestCase(
    "SHA-384",
    "P-384",
    msg,
    nTests,
  ...
  var biasTest521 = new EcdsaBiasTestCase(
    "SHA-512",
    "P-521",
    msg,
    nTests,
