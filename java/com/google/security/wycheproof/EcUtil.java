...
/**
 * Some utilities for testing Elliptic curve crypto. This code is for testing
 * only and hasn't been reviewed for production.
 */
public class EcUtil {
  /**
   * Returns the ECParameterSpec for a named curve. Not every provider
   * implements the AlgorithmParameters. Therefore, most tests use alternative
   * functions.
   */
  ...
  /**
   * Returns the bit size of a given curve. TODO(bleichen): add all curves that
   * are tested.
   */
  ...
  /**
   * Returns the ECParameterSpec for a named curve. Only a handful curves that
   * are used in the tests are implemented.
   */
  public static ECParameterSpec getCurveSpecRef(String name)
  ...
  public static ECParameterSpec getNistCurveSpec(String decimalP,
                                                 String decimalN, String hexB,
    ...
    java.security.spec.EllipticCurve curveSpec =
        new java.security.spec.EllipticCurve(fp, a, b);
  ...
  public static ECParameterSpec getBrainpoolP224r1Params() {
    ...
    BigInteger x = new BigInteger(
        "0D9029AD2C7E5CF4340823B2A87DC68C9E4CE3174C1E6EFDEE12C07D", 16);
    BigInteger y = new BigInteger(
        "58AA56F772C0726F24C6B89E4ECDAC24354B9E99CAA3F6D3761402CD", 16);
  ...
  /**
   * Compute the Legendre symbol of x mod p. This implementation is slow. Faster
   * would be the computation for the Jacobi symbol.
   *
   * @param x an integer
   * @param p a prime modulus
   * @returns 1 if x is a quadratic residue, -1 if x is a non-quadratic residue
   *     and 0 if x and p are not coprime.
   * @throws GeneralSecurityException when the computation shows that p is not
   *     prime.
   */
  ...
  /**
   * Computes a modular square root. Timing and exceptions can leak information
   * about the inputs. Therefore this method must only be used in tests.
   *
   * @param x the square
  ...
  public static BigInteger modSqrt(BigInteger x, BigInteger p)
    ...
    if (p.testBit(0) && p.testBit(1)) {
      ...
      // This alogorithm is preferrable to Tonelli-Shanks for primes p where p-1
      // is divisible by a large power of 2, which is a frequent choice since it
      // simplifies modular reduction.
      BigInteger a = BigInteger.ONE;
      BigInteger d = null;
      while (true) {
        d = a.multiply(a).subtract(x).mod(p);
        // Computes the Legendre symbol. Using the Jacobi symbol would be a
        // faster. Using Legendre has the advantage, that it detects a non prime
        // p with high probability. On the other hand if p = q^2 then the Jacobi
        // (d/p)==1 for almost all d's and thus using the Jacobi symbol here can
        // result in an endless loop with invalid inputs.
        int t = legendre(d, p);
        if (t == -1) {
          break;
        } else {
      ...
      // Thus if n is square then (a+sqrt(d))^((p+1)/2) (mod p) is a square root
      // of n.
    ...
    // The methods used to compute the square root only guarantee a correct
    // result if the preconditions (i.e. p prime and x is a square) are
    // satisfied. Otherwise the value is undefined. Hence, it is important to
    // verify that squareRoot is indeed a square root.
    if (squareRoot != null &&
        squareRoot.multiply(squareRoot).mod(p).compareTo(x) != 0) {
  ...
  /**
   * Returns the size of an element of the field over which the curve is
   * defined.
   *
   * @param curve must be a prime order elliptic curve
  ...
  /**
   * Checks that a point is on a given elliptic curve. This method implements
   * the partial public key validation routine from Section 5.6.2.6 of NIST SP
   * 800-56A
   * http://csrc.nist.gov/publications/nistpubs/800-56A/SP800-56A_Revision1_Mar08-2007.pdf
   * A partial public key validation is sufficient for curves with cofactor 1.
   * See Section B.3 of
   * http://www.nsa.gov/ia/_files/SuiteB_Implementer_G-113808.pdf The point
   * validations above are taken from recommendations for ECDH, because
   * parameter checks in ECDH are much more important than for the case of
   * ECDSA. Performing this test for ECDSA keys is mainly a sanity check.
   *
   * @param point the point that needs verification
   * @param ec the elliptic curve. This must be a curve over a prime order
   *     field.
   * @throws GeneralSecurityException if the field is binary or if the point is
   *     not on the curve.
   */
  public static void checkPointOnCurve(ECPoint point, EllipticCurve ec)
    ...
    BigInteger rhs =
        x.multiply(x).add(ec.getA()).multiply(x).add(ec.getB()).mod(p);
    if (!lhs.equals(rhs)) {
  ...
  /**
   * Checks a public key. I.e. this checks that the point defining the public
   * key is on the curve.
   *
   ...
   */
  public static void checkPublicKey(ECPublicKey key)
      throws GeneralSecurityException {
   ...
   * @param ecParams contains the curve of the point. This must be over a prime
   *     order field.
   */
  public static ECPoint getPoint(BigInteger x, boolean bit0,
   ...
   *
   * @param bytes The compressed point. Its representation is z || x where z is
   *     2+lsb(y) and x is using a unsigned fixed length big-endian
   *     representation.
   * @param ecParams the specification of the curve. Only Weierstrass curves
   *     over prime order fields are implemented.
   */
  public static ECPoint decompressPoint(byte[] bytes, ECParameterSpec ecParams)
    ...
    BigInteger x =
        new BigInteger(1, Arrays.copyOfRange(bytes, 1, bytes.length));
    if (x.compareTo(BigInteger.ZERO) == -1 || x.compareTo(p) != -1) {
      throw new GeneralSecurityException("x is out of range");
    }
    // Compute rhs == x^3 + a x + b (mod p)
  ...
  /**
   * Returns a weak public key of order 3 such that the public key point is on
   * the curve specified in ecParams. This method is used to check ECC
   * implementations for missing step in the verification of the public key.
   * E.g. implementations of ECDH must verify that the public key contains a
   * point on the curve as well as public and secret key are using the same
   * curve.
   *
   * @param ecParams the parameters of the key to attack. This must be a curve
   *     in Weierstrass form over a prime order field.
