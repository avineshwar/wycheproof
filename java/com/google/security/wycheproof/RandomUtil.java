...
/**
 * A collection of utilities for testing random number generators. So far this
 * util simply checks that random numbers are not generated by java.util.Random.
 * Eventually we plan to add detection for other random number generators too.
 *
...
public class RandomUtil {
  ...
  /**
   * Computes a seed that would initialize a java.util.Random object with a
   * given state.
   */
  protected static long getSeedForState(long seed) { return seed ^ A; }
  ...
  /**
   * Given two subsequent outputs x0 and x1 from java.util.Random this function
   * computes the internal state of java.util.Random after returning x0 or
  ...
  /**
   * Attempts to find a seed such that it generates the prime p. Returns -1 if
   * no seed is found.
   */
  ...
  /**
   * Checks whether p is a random prime. A prime generated with a secure random
   * number generator passes with probability > 1-2^{-32}. No checks are
   * performed for primes smaller than 96 bits.
   *
   * @throws GeneralSecurityException if the prime was generated using
   *     java.util.Random
   */
  static void checkPrime(BigInteger p) throws GeneralSecurityException {
    ...
    if (seed != -1) {
      throw new GeneralSecurityException("java.util.Random with seed " + seed +
                                         " was likely used to generate prime");
    }
  }
}
