...
public class JsonMacTest {
   ...
   * @return the tag
   * @throws GeneralSecurityException if the algorithm or the parameter sizes
   *     are not supported or if the initialization failed. For example one case
   *     are GMACs with a tag size othe than 128 bits, since the JCE interface
   *     does not seem to support such a specification.
   */
  protected static byte[] computeMac(String algorithm, byte[] key, byte[] msg,
                                     int tagSize)
    ...
    if (algorithm.startsWith("HMAC")) {
      ...
      //     return mac.doFinal();
      //   </pre>
      //   But this class is often not supported. Hence the computation here,
      //   just computes a full length tag and truncates it. The drawback of
      //   having to truncate tags is that the caller has to compare truncated
      //   tags during verification.
      mac.init(keySpec);
      mac.update(msg);
  ...
  /**
   * Tests a randomized MAC (i.e. a message authetication that takes an
   * additional IV as parameter) against test vectors.
   *
    ...
    for (JsonElement g : test.getAsJsonArray("testGroups")) {
      ...
      for (JsonElement t : group.getAsJsonArray("tests")) {
        ...
        try {
          ...
          // Thrown by javax.crypto.spec.SecretKeySpec (e.g. when the key is
          // empty).
          continue;
        }
        ...
        if (result.equals("invalid")) {
          if (eq) {
            ...
            // E.g. an implementation must not allow AES-GMAC with an IV of
            // length 0, since this leaks the authentication key.
          ...
          if (eq) {
            ...
            System.out.println("Incorrect tag for " + tc +
                               " expected:" + TestUtil.bytesToHex(expectedTag) +
                               " computed:" + TestUtil.bytesToHex(computedTag));
            errors++;
          }
        }
      }
   ...
   *     does not seem to support such a specification.
   */
  protected static Mac getInitializedMacWithIv(String algorithm, byte[] key,
                                               byte[] iv, int tagSize)
      throws GeneralSecurityException {
    Mac mac = Mac.getInstance(algorithm);
    algorithm = algorithm.toUpperCase(Locale.ENGLISH);
    if (algorithm.equals("AES-GMAC")) {
      ...
      if (tagSize != 128) {
        throw new InvalidAlgorithmParameterException(
            "only 128-bit tag is supported");
      }
      IvParameterSpec params = new IvParameterSpec(iv);
      // TODO(bleichen): I'm unaware of a method that allows to specify the tag
      // size in JCE.
      //   E.g. the following parameter specification does not work (at least
      //   not in BC): GCMParameterSpec params = new GCMParameterSpec(tagSize,
      //   iv);
      mac.init(keySpec, params);
      return mac;
    } else {
   ...
   */
  public void testMacWithIv(String filename, String algorithm)
      throws Exception {
    // Checking preconditions.
    try {
      Mac.getInstance(algorithm);
