...
public class RsaEncryptionTest {
  ...
  /**
   * Providers that implement RSA with PKCS1Padding but not OAEP are outdated
   * and should be avoided even if RSA is currently not used in a project. Such
   * providers promote using an insecure cipher. There is a great danger that
   * PKCS1Padding is used as a temporary workaround, but later stays in the
   * project for much longer than necessary.
   */
  @Test
  public void testOutdatedProvider() throws Exception {
    try {
      ...
      try {
        ...
        fail("Provider " + c.getProvider().getName() +
  ...
  protected static PrivateKey getPrivateKey(JsonObject object)
    ...
    byte[] encoded =
        TestUtil.hexToBytes(object.get("privateKeyPkcs8").getAsString());
   ...
   * RSA-PKCS #1 v 1.5 is susceptible to chosen ciphertext attacks. The
   * seriousness of the attack depends on how much information is leaked when
   * decrypting an invalid ciphertext. The test vectors with invalid padding
   * contain a flag "InvalidPkcs1Padding". The test below expects that all test
   * vectors with this flag throw an indistinguishable exception.
   *
   * <p><b>References:</b>
   *
   * <ul>
   *   <li>Bleichenbacher, "Chosen ciphertext attacks against protocols based on
   * the RSA encryption standard PKCS# 1" Crypto 98 <li>Manger, "A chosen
   * ciphertext attack on RSA optimal asymmetric encryption padding (OAEP) as
   * standardized in PKCS# 1 v2.0", Crypto 2001 This paper shows that OAEP is
   * susceptible to a chosen ciphertext attack if error messages distinguish
   * between different failure condidtions. <li>Bardou, Focardi, Kawamoto,
   * Simionato, Steel, Tsay "Efficient Padding Oracle Attacks on Cryptographic
   * Hardware", Crypto 2012 The paper shows that small differences on what
   *       information an attacker receives can make a big difference on the
   * number of chosen message necessary for an attack. <li>Smart, "Errors
   * matter: Breaking RSA-based PIN encryption with thirty ciphertext validity
   *       queries" RSA conference, 2010 This paper shows that padding oracle
   * attacks can be successful with even a small number of queries.
   * </ul>
   *
   * <p><b>Some recent bugs:</b> CVE-2012-5081: Java JSSE provider leaked
   * information through exceptions and timing. Both the PKCS #1 padding and the
   * OAEP padding were broken:
   * http://www-brs.ub.ruhr-uni-bochum.de/netahtml/HSS/Diss/MeyerChristopher/diss.pdf
   *
   * <p><b>What this test does not (yet) cover:</b>
   *
   * <ul>
   *   <li>A previous version of one of the provider leaked the block type.
   * (when was this fixed?) <li>Some attacks require a large number of
   * ciphertexts to be detected if random ciphertexts are used. Such problems
   * require specifically crafted ciphertexts to run in a unit test. E.g.
   * "Attacking RSA-based Sessions in SSL/TLS" by V. Klima, O. Pokorny, and T.
   * Rosa: https://eprint.iacr.org/2003/052/ <li>Timing leakages because of
   * differences in parsing the padding (e.g. CVE-2015-7827) Such differences
   * are too small to be reliably detectable in unit tests.
   * </ul>
   */
  @SuppressWarnings("InsecureCryptoUsage")
  public void testDecryption(String filename) throws Exception {
    ...
    // Padding oracle attacks become simpler when the decryption leaks detailed
    // information about invalid paddings. Hence implementations are expected to
    // not include such information in the exception thrown in the case of an
    // invalid padding. Test vectors with an invalid padding have a flag
    // "InvalidPkcs1Padding". Invalid test vectors without this flag are cases
    // where the error are detected before the ciphertext is decrypted, e.g. if
    // the size of the ciphertext is incorrect.
