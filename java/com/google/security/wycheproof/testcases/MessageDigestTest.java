 ...
 * vectors for long messages are available. The paper "Finding Bugs in
 * Cryptographic Hash Function Implementations" by Mouha, Raunak, Kuhn, and
 * Kacker, https://eprint.iacr.org/2017/891.pdf contains an analysis of
 * implementations submitted to the SHA-3 competition. Many of the
...
public class MessageDigestTest {
  ...
  private void testUpdate(String algorithm, int messageLength)
      throws Exception {
    ...
    for (int i = 0; i <= messageLength; i++) {
      ...
      if (!Arrays.equals(digest1, digest2)) {
        fail("Different hashes for same input digest1:" +
             TestUtil.bytesToHex(digest1) +
             " digest2:" + TestUtil.bytesToHex(digest2) + " i:" + i);
      }
    }
  }

  /** Same test as above using ByteBuffers instead. */
  private void testByteBuffer(String algorithm, int messageLength)
      throws Exception {
    SecureRandom rand = new SecureRandom();
    byte[] message = new byte[messageLength];
    rand.nextBytes(message);
    ...
    for (int i = 0; i <= messageLength; i++) {
      ...
      ByteBuffer part2 =
          ByteBuffer.wrap(message, i, message.length - i).slice();
      md.update(part1);
      md.update(part2);
      byte[] digest2 = md.digest();
      if (!Arrays.equals(digest1, digest2)) {
  ...
  /**
   * Same test as above using readonly ByteBuffers instead. If a ByteBuffer is
   * readonly, then the underlying array is not accessible. Some providers might
   * have distinct code for ByteBuffers where the underlying array is accessible
   * and where the underlying array is hidden.
   */
  public void testReadOnlyByteBuffer(String algorithm, int messageLength)
      throws Exception {
    SecureRandom rand = new SecureRandom();
    byte[] message = new byte[messageLength];
    rand.nextBytes(message);
    ...
    for (int i = 0; i <= messageLength; i++) {
      ...
      ByteBuffer part2 =
          ByteBuffer.wrap(message, i, message.length - i).asReadOnlyBuffer();
      md.update(part1);
      md.update(part2);
      byte[] digest2 = md.digest();
      if (!Arrays.equals(digest1, digest2)) {
  ...
  /**
   * Some provider allow to clone instances of MessageDigests. This test checks
   * whether cloning modifies the hash result.
   */
  public void testClone(String algorithm, int messageLength) throws Exception {
    ...
    for (int i = 0; i <= messageLength; i++) {
      ...
      try {
        ...
        System.out.println("Cloning " + algorithm +
                           " is not supported. Skipping test.");
        return;
      }
    }

    // A variant of the test above that checks whether the source message digest
    // is still usable.
    md.reset();
    for (int i = 0; i < messageLength; i++) {
      try {
   ...
   * @return the digest
   * @throws GeneralSecurityException if the computation of the hash fails (e.g.
   *     because the algorithm is unknown).
   */
  public byte[] hashRepeatedMessage(String algorithm, byte[] message,
   ...
   * it is still possible to compute a hash of a long message by repeatedly
   * calling MessageDigest.update(). To compute correct hashes the total message
   * length must be known. This length can be bigger than 2^32 bytes.
   *
  ...
  private void testLongMessage(String algorithm, String message,
                               long repetitions, String expected)
      throws Exception {
    try {
      MessageDigest.getInstance(algorithm);
    } catch (NoSuchAlgorithmException ex) {
  ...
  @Test
  public void
  testLongMessageMd5() throws Exception {
    testLongMessage("MD5", "a", 2147483647L,
                    "bb2ef53aae423cb9fbf8788f187601e6");
    testLongMessage("MD5", "a", 5000000000L,
                    "cf3147924864955e385804daee42d3ef");
  }
  ...
  @Test
  public void
  testLongMessageSha1() throws Exception {
    testLongMessage("SHA-1", "a", 2147483647L,
                    "1e5b490b10255e37fd96d0964f2fbfb91ed47536");
    testLongMessage("SHA-1", "a", 5000000000L,
                    "109b426b74c3dc1bd0e15d3524c5b837557647f2");
  }
  ...
  @Test
  public void
  testLongMessageSha256() throws Exception {
    testLongMessage(
        "SHA-256", "a", 2147483647L,
        "6cc47f3907eea90fb8de9493cf025923fff2b88fcac896cbf38036d5913b6bed");
    testLongMessage(
        "SHA-256", "a", 5000000000L,
  ...
  @Test
  public void
  testLongMessageSha224() throws Exception {
    testLongMessage("SHA-224", "a", 2147483647L,
                    "bf5dbff84919d0bd40316439d102c6f856553b7a89ef9212fd200d9e");
    testLongMessage("SHA-224", "a", 5000000000L,
  ...
  @Test
  public void
  testLongMessageSha512() throws Exception {
    testLongMessage(
        "SHA-512", "a", 2147483647L,
  ...
  @Test
  public void
  testLongMessageSha3_224() throws Exception {
    testLongMessage("SHA3-224", "a", 2147483647L,
                    "24abc6b4055cea68422fa8d73031f45f73f2afda09be9c0dae2ab88e");
    testLongMessage("SHA3-224", "a", 5000000000L,
                    "96ce1138a9f42ba22929594a636404c13a99fe3c31a05fe3a00a8fda");
  }
  ...
  @Test
  public void
  testLongMessageSha3_256() throws Exception {
    testLongMessage(
        "SHA3-256", "a", 2147483647L,
        "8bcd31a0d849cca71991062525ffe8b5dd07b41f686880e6c30bfe4382bb2beb");
    testLongMessage(
        "SHA3-256", "a", 5000000000L,
  ...
  @Test
  public void
  testLongMessageSha3_512() throws Exception {
    testLongMessage(
        "SHA3-512", "a", 2147483647L,
