...
/**
 * This test uses test vectors in JSON format to check digital signature
 * schemes. There are still a lot of open questions, e.g. the format for the
 * test vectors is not yet finalized. Therefore, we are not integrating the
 * tests here into other tests
 */
@RunWith(JUnit4.class)
public class JsonSignatureTest {

  /**
   * Defines the format of the signatures. RAW is used when the signature scheme
   * already defines an encoding (e.g. this is used for RSA signatures).
   */
  ...
  /**
   * Convert hash names, so that they can be used in an algorithm name for a
   * signature. The algorithm names used in JCA are a bit inconsequential. E.g.
   * a dash is necessary for message digests (e.g. "SHA-256") but are not used
   * in the corresponding names for digital signatures (e.g. "SHA256WITHECDSA").
   * Providers sometimes use distinct algorithm names for the same cryptographic
   * primitive. On the other hand, the dash remains for SHA-3. Hence, the
   * correct name for ECDSA with SHA3-256 is "SHA3-256WithECDSA".
   *
   * <p>See
  ...
  /**
   * Returns an instance of java.security.Signature for an algorithm name, a
   * digest name and a signature format.
   *
   * @param md the name of the message digest (e.g. "SHA-256")
   * @param signatureAlgorithm the name of the signature algorithm (e.g.
   *     "ECDSA")
   ...
   */
  protected static Signature getSignatureInstance(JsonObject group,
    ...
    if (signatureAlgorithm.equals("ECDSA") ||
        signatureAlgorithm.equals("DSA")) {
      if (signatureFormat == Format.ASN) {
        return Signature.getInstance(md + "WITH" + signatureAlgorithm);
      } else if (signatureFormat == Format.P1363) {
        // The algorithm names for signature schemes with P1363 format have
        // distinct names in distinct providers. This is mainly the case since
        // the P1363 format has only been added in jdk11, while providers such
        // as BouncyCastle added the format earlier than that. Hence the code
        // below just tries known algorithm names.
        try {
    ...
    } else if (signatureAlgorithm.equals("ED25519") ||
               signatureAlgorithm.equals("ED448")) {
      if (signatureFormat == Format.RAW) {
    ...
    throw new NoSuchAlgorithmException("Algorithm " + signatureAlgorithm +
                                       " with format " + signatureFormat +
                                       " is not supported");
  }

  /**
   * Returns the expected JSON schema for a given test or "" if the schema is
   * undefined. The purpose of this function is to perform a sanity test with
   * the goal to recognize incorrect test setups.
   * @param signatureAlgorithm the signataure algorithm (e.g. "ECDSA")
   * @param signatureFormat the format of the signatures
   * @param verify true if verification is tested, false if signature
   *     generations is tested.
   */
  protected static String expectedSchema(String signatureAlgorithm,
                                         Format signatureFormat,
                                         boolean verify) {
    if (verify) {
      if (signatureAlgorithm.equals("ECDSA")) {
        ...
        case ASN:
          return "ecdsa_verify_schema.json";
        case P1363:
          return "ecdsa_p1363_verify_schema.json";
        default:
          break;
        }
        ...
        switch (signatureFormat) {
        case ASN:
          return "dsa_verify_schema.json";
        case P1363:
          return "dsa_p1363_verify_schema.json";
        default:
          break;
        }
      } else if (signatureAlgorithm.equals("RSA")) {
        // Only RSA-PKCS1 is implemented in this unit test.
        // RSA-PSS signatures have their own unit test, because the algorithm
        // parameters require a setup that is a little different.
        switch (signatureFormat) {
        case RAW:
          return "rsassa_pkcs1_verify_schema.json";
        default:
          break;
        }
      } else if (signatureAlgorithm.equals("ED25519") ||
    ...
    // If the schema is not defined then the tests below still run. The only
    // drawback is that incorrect test setups are not recognized and will
    // probably lead to failures later.
    return "";
  }
  /**
  ...
  protected static PublicKey getPublicKey(JsonObject group, String algorithm)
      throws Exception {
    KeyFactory kf;
  ...
  protected static PrivateKey getPrivateKey(JsonObject object, String algorithm)
    ...
    if (algorithm.equals("RSA")) {
      ...
      byte[] encoded =
          TestUtil.hexToBytes(getString(object, "privateKeyPkcs8"));
   ...
   *         "type": "ECPublicKey",
   *         "wx":
   "0c9c4bc2617c81eb2dcbfda2db2a370a955be86a0d2e95fcb86a99f90cf046573",
   *         "wy":
   "0c400363b1b6bcc3595a7d6d3575ccebcbb03f90ba8e58da2bc4824272f4fecff"
   *       },
   ...
   * @param signatureFormat the format of the signatures. This should be
   Format.P1363 for
   *        P1363 encoded signatures Format.ASN for ASN.1 encoded signature  and
   Format.RAW otherwise.
   * @param allowSkippingKeys if true then keys that cannot be constructed will
   not fail the test.
   *     This is for example used for files with test vectors that use elliptic
   curves that are not
   *     commonly supported.
   *
 */
  public void testVerification(String filename, String signatureAlgorithm,
                               Format signatureFormat,
                               boolean allowSkippingKeys) throws Exception {
    JsonObject test = JsonUtil.getTestVectors(filename);
    // Checks whether the test vectors in the file use the expected algorithm
    // and the expected format for the signatures.
    ...
    if (!schema.isEmpty() && !schema.equals(actualSchema)) {
      System.out.println(signatureAlgorithm +
                         ": expecting test vectors with schema " + schema +
                         " found vectors with schema " + actualSchema);
    }
    ...
    for (JsonElement g : test.getAsJsonArray("testGroups")) {
      ...
      try {
        verifier =
            getSignatureInstance(group, signatureAlgorithm, signatureFormat);
      } catch (NoSuchAlgorithmException ex) {
        if (!allowSkippingKeys) {
          throw ex;
        }
      ...
      for (JsonElement t : group.getAsJsonArray("tests")) {
        ...
        try {
          ...
          // We don't flag these cases and simply consider the signature as
          // invalid.
          verified = false;
          failure = ex;
        } catch (java.lang.ArithmeticException ex) {
          // b/33446454 The Sun provider may throw an ArithmeticException
          // instead of the expected SignatureException for DSA signatures. We
          ...
          // exceptions.
          System.out.println(signatureAlgorithm + " signature throws " +
                             ex.toString() + " " + filename + " tcId:" + tcid +
                             " sig:" + sig);
          verified = false;
          failure = ex;
          errors++;
        ...
        if (!verified && result.equals("valid")) {
          ...
          System.out.println(
              "Valid " + signatureAlgorithm + " signature not verified."
              + " " + filename + " tcId:" + tcid + " sig:" + sig + reason);
          errors++;
        } else if (verified) {
          if (result.equals("invalid")) {
            System.out.println(
                "Invalid" + signatureAlgorithm + " signature verified."
                + " " + filename + " tcId:" + tcid + " sig:" + sig);
            errors++;
          } else {
            verifiedSignatures++;
          }
    ...
    if (skippedKeys > 0 || skippedAlgorithms > 0 || verifiedSignatures == 0) {
      System.out.println("File:" + filename +
                         " number of skipped keys:" + skippedKeys +
                         " number of skipped algorithms:" + skippedAlgorithms +
                         " number of supported keys:" + supportedKeys +
                         " verified signatures:" + verifiedSignatures);
  ...
  /**
   * Tests signature generation of deterministic signature schemes such as
   * RSA-PKCS#1 v1.5.
   *
   * <p>The test expects that signatures are fully complying with the standards.
   * E.g. it is acceptable when RSA-PKCS#1 verification considers ASN encodings
   * of the digest name with a missing NULL value for legacy reasons. However,
   * it is considered not acceptable when the signature generation does not
   * include the NULL value.
   *
   * @param filename the filename of the test vectors
   * @param signatureAlgorithm the algorithm name of the test vectors (e.g.
   *     "RSA")
   * @param signatureFormat the format of the signatures.
   * @param allowSkippingKeys if true then keys that cannot be constructed will
   *     not fail the test.
   */
  public void testSigning(String filename, String signatureAlgorithm,
                          Format signatureFormat, boolean allowSkippingKeys)
      throws Exception {
    JsonObject test = JsonUtil.getTestVectors(filename);
    // Checks whether the test vectors in the file use the expected algorithm
    // and the expected format for the signatures.
      ...
      for (JsonElement t : group.getAsJsonArray("tests")) {
        ...
        try {
          ...
          if (!sig.equals(expectedSig)) {
            System.out.println("Incorrect signature generated " + filename +
                               " tcId:" + tcid + " expected:" + expectedSig +
                               " sig:" + sig);
            errors++;
          } else {
            cntTests++;
          ...
          if (result.equals("valid")) {
            System.out.println("Failed to sign " + filename + " tcId:" + tcid +
                               " with exception:" + ex);
  ...
  public void testSecp224r1Sha224() throws Exception {
    testVerification("ecdsa_secp224r1_sha224_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp224r1Sha256() throws Exception {
    testVerification("ecdsa_secp224r1_sha256_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp224r1Sha512() throws Exception {
    testVerification("ecdsa_secp224r1_sha512_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp256r1Sha256() throws Exception {
    testVerification("ecdsa_secp256r1_sha256_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp256r1Sha512() throws Exception {
    testVerification("ecdsa_secp256r1_sha512_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp384r1Sha384() throws Exception {
    testVerification("ecdsa_secp384r1_sha384_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp384r1Sha512() throws Exception {
    testVerification("ecdsa_secp384r1_sha512_test.json", "ECDSA", Format.ASN,
                     false);
  }

  @Test
  public void testSecp521r1Sha512() throws Exception {
    testVerification("ecdsa_secp521r1_sha512_test.json", "ECDSA", Format.ASN,
                     false);
  }
  ...
  public void testSecp256k1Sha256() throws Exception {
    testVerification("ecdsa_secp256k1_sha256_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp256k1Sha512() throws Exception {
    testVerification("ecdsa_secp256k1_sha512_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @NoPresubmitTest(providers = {ProviderType.OPENJDK}, bugs = {"b/117643131"})
  @Test
  public void testBrainpoolP224r1Sha224() throws Exception {
    testVerification("ecdsa_brainpoolP224r1_sha224_test.json", "ECDSA",
                     Format.ASN, true);
  }

  @Test
  public void testBrainpoolP256r1Sha256() throws Exception {
    testVerification("ecdsa_brainpoolP256r1_sha256_test.json", "ECDSA",
                     Format.ASN, true);
  }

  @Test
  public void testBrainpoolP320r1Sha384() throws Exception {
    testVerification("ecdsa_brainpoolP320r1_sha384_test.json", "ECDSA",
                     Format.ASN, true);
  }

  @Test
  public void testBrainpoolP384r1Sha384() throws Exception {
    testVerification("ecdsa_brainpoolP384r1_sha384_test.json", "ECDSA",
                     Format.ASN, true);
  }

  @Test
  public void testBrainpoolP512r1Sha512() throws Exception {
    testVerification("ecdsa_brainpoolP512r1_sha512_test.json", "ECDSA",
                     Format.ASN, true);
  }
  ...
  public void testSecp224r1Sha3_224() throws Exception {
    testVerification("ecdsa_secp224r1_sha3_224_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp224r1Sha3_256() throws Exception {
    testVerification("ecdsa_secp224r1_sha3_256_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp224r1Sha3_512() throws Exception {
    testVerification("ecdsa_secp224r1_sha3_512_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp256r1Sha3_256() throws Exception {
    testVerification("ecdsa_secp256r1_sha3_256_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp256r1Sha3_512() throws Exception {
    testVerification("ecdsa_secp256r1_sha3_512_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp256k1Sha3_256() throws Exception {
    testVerification("ecdsa_secp256k1_sha3_256_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp256k1Sha3_512() throws Exception {
    testVerification("ecdsa_secp256k1_sha3_512_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp384r1Sha3_384() throws Exception {
    testVerification("ecdsa_secp384r1_sha3_384_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp384r1Sha3_512() throws Exception {
    testVerification("ecdsa_secp384r1_sha3_512_test.json", "ECDSA", Format.ASN,
                     true);
  }

  @Test
  public void testSecp521r1Sha3_512() throws Exception {
    testVerification("ecdsa_secp521r1_sha3_512_test.json", "ECDSA", Format.ASN,
                     true);
  }
  ...
  public void testSecp224r1Sha224inP1363Format() throws Exception {
    testVerification("ecdsa_secp224r1_sha224_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp224r1Sha256inP1363Format() throws Exception {
    testVerification("ecdsa_secp224r1_sha256_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp224r1Sha512inP1363Format() throws Exception {
    testVerification("ecdsa_secp224r1_sha512_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp256r1Sha256inP1363Format() throws Exception {
    testVerification("ecdsa_secp256r1_sha256_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp256r1Sha512inP1363Format() throws Exception {
    testVerification("ecdsa_secp256r1_sha512_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp384r1Sha384inP1363Format() throws Exception {
    testVerification("ecdsa_secp384r1_sha384_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp384r1Sha512inP1363Format() throws Exception {
    testVerification("ecdsa_secp384r1_sha512_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp521r1Sha512inP1363Format() throws Exception {
    testVerification("ecdsa_secp521r1_sha512_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp256k1Sha256inP1363Format() throws Exception {
    testVerification("ecdsa_secp256k1_sha256_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testSecp256k1Sha512inP1363Format() throws Exception {
    testVerification("ecdsa_secp256k1_sha512_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }
  ...
  public void testBrainpoolP224r1Sha224inP1363Format() throws Exception {
    testVerification("ecdsa_brainpoolP224r1_sha224_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testBrainpoolP256r1Sha256inP1363Format() throws Exception {
    testVerification("ecdsa_brainpoolP256r1_sha256_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testBrainpoolP320r1Sha384inP1363Format() throws Exception {
    testVerification("ecdsa_brainpoolP320r1_sha384_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testBrainpoolP384r1Sha384inP1363Format() throws Exception {
    testVerification("ecdsa_brainpoolP384r1_sha384_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }

  @Test
  public void testBrainpoolP512r1Sha512inP1363Format() throws Exception {
    testVerification("ecdsa_brainpoolP512r1_sha512_p1363_test.json", "ECDSA",
                     Format.P1363, true);
  }
  ...
  public void testRsaSignature2048sha224() throws Exception {
    testVerification("rsa_signature_2048_sha224_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures2048sha256() throws Exception {
    testVerification("rsa_signature_2048_sha256_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures2048sha384() throws Exception {
    testVerification("rsa_signature_2048_sha384_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures2048sha512() throws Exception {
    testVerification("rsa_signature_2048_sha512_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures3072sha256() throws Exception {
    testVerification("rsa_signature_3072_sha256_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures3072sha384() throws Exception {
    testVerification("rsa_signature_3072_sha384_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures3072sha512() throws Exception {
    testVerification("rsa_signature_3072_sha512_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures4096sha384() throws Exception {
    testVerification("rsa_signature_4096_sha384_test.json", "RSA", Format.RAW,
                     false);
  }

  @Test
  public void testRsaSignatures4096sha512() throws Exception {
    testVerification("rsa_signature_4096_sha512_test.json", "RSA", Format.RAW,
                     false);
  }
  ...
  public void testRsaSignatures2048sha512_224() throws Exception {
    testVerification("rsa_signature_2048_sha512_224_test.json", "RSA",
                     Format.RAW, true);
  }

  @Test
  public void testRsaSignatures2048sha512_256() throws Exception {
    testVerification("rsa_signature_2048_sha512_256_test.json", "RSA",
                     Format.RAW, true);
  }

  @Test
  public void testRsaSignatures3072sha512_256() throws Exception {
    testVerification("rsa_signature_3072_sha512_256_test.json", "RSA",
                     Format.RAW, true);
  }

  @Test
  public void testRsaSignatures4096sha512_256() throws Exception {
    testVerification("rsa_signature_4096_sha512_256_test.json", "RSA",
                     Format.RAW, true);
  }

  // RSA signatures with SHA-3. Not every provider supports SHA-3. Hence the
  // tests may be skipped.
  @Test
  public void testRsaSignature2048sha3_224() throws Exception {
    testVerification("rsa_signature_2048_sha3_224_test.json", "RSA", Format.RAW,
                     true);
  }

  @Test
  public void testRsaSignatures2048sha3_256() throws Exception {
    testVerification("rsa_signature_2048_sha3_256_test.json", "RSA", Format.RAW,
                     true);
  }

  @Test
  public void testRsaSignatures2048sha3_512() throws Exception {
    testVerification("rsa_signature_2048_sha3_512_test.json", "RSA", Format.RAW,
                     true);
  }

  @Test
  public void testRsaSignatures3072sha3_256() throws Exception {
    testVerification("rsa_signature_3072_sha3_256_test.json", "RSA", Format.RAW,
                     true);
  }

  @Test
  public void testRsaSignatures3072sha3_384() throws Exception {
    testVerification("rsa_signature_3072_sha3_384_test.json", "RSA", Format.RAW,
                     true);
  }

  @Test
  public void testRsaSignatures3072sha3_512() throws Exception {
    testVerification("rsa_signature_3072_sha3_512_test.json", "RSA", Format.RAW,
                     true);
  }
  ...
  @Test
  public void
  testEd25519Verify() throws Exception {
  ...
  @Test
  public void
  testEd448Verify() throws Exception {
  ...
  @ExcludedTest(providers = {ProviderType.CONSCRYPT},
                comment = "Conscrypt does not support DSA.")
  @Test
  public void
  testDsa2048Sha224() throws Exception {
  ...
  @Test
  public void
  testDsa2048JdkSha256() throws Exception {
  ...
  @Test
  public void
  testDsa2048Sha256() throws Exception {
  ...
  @Test
  public void
  testDsa3072Sha256() throws Exception {
  ...
  @Test
  public void
  testDsa2048Sha224inP1363Format() throws Exception {
    testVerification("dsa_2048_224_sha224_p1363_test.json", "DSA", Format.P1363,
                     true);
  }
  ...
  @Test
  public void
  testDsa2048JdkSha256inP1363Format() throws Exception {
    testVerification("dsa_2048_224_sha256_p1363_test.json", "DSA", Format.P1363,
                     true);
  }
  ...
  @Test
  public void
  testDsa2048Sha256inP1363Format() throws Exception {
    testVerification("dsa_2048_256_sha256_p1363_test.json", "DSA", Format.P1363,
                     true);
  }
  ...
  @Test
  public void
  testDsa3072Sha256inP1363Format() throws Exception {
    testVerification("dsa_3072_256_sha256_p1363_test.json", "DSA", Format.P1363,
                     true);
  }
}
