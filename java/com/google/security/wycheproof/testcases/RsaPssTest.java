...
public class RsaPssTest {
   ...
   * @param sha the name of the hash function for hashing the input (e.g.
   *     "SHA-256")
   * @param mgf the name of the mask generating function (typically "MGF1")
   * @param mgfSha the name of the hash function for the mask generating
   *     function
   *        (typically the same as sha).
   * @param saltLength the length of the salt in bytes (typically the digest
   *     size of sha,
   *        i.e. 32 for "SHA-256")
  ...
  getPssAlgorithmParameters(int keySizeInBits, String sha, String mgf,
                            String mgfSha, int saltLength) throws Exception {
    BigInteger publicExponent = new BigInteger("65537");
    PSSParameterSpec params = new PSSParameterSpec(
        sha, mgf, new MGF1ParameterSpec(mgfSha), saltLength, 1);
    // Uses reflection to call
    // public RSAKeyGenParameterSpec(int keysize, BigInteger publicExponent,
    //        AlgorithmParameterSpec keyParams)
    // because this method is only supported in jdk11. This throws a
    // NoSuchMethodException for older jdks.
  ...
  public void testEncodeDecodePublic() throws Exception {
    ...
    // Tries to generate another pair or keys. This time the generator is given
    // an RSAKeyGenParameterSpec containing the key size an the PSS parameters.
    String sha = "SHA-256";
    String mgf = "MGF1";
    int saltLength = 20;
    try {
   ...
   *
   * @param algorithm the algorithm name for an RSA-PSS instance. (e.g.
   *     "SHA256WithRSAandMGF1")
   * @param expectedHash the hash algorithm expected for the given algorithm
   * @param expectedMgf the mask generation function expected for the given
   *     algorithm (e.g. "MGF1")
   * @param expectedMgfHash the hash algorithm exptected for the mask generation
   *     function
   * @param expectedSaltLength the expected salt length in bytes for the given
   *     algorithm
   * @param expectedTrailerField the expected value for the tailer field (e.g. 1
   *     for 0xbc).
   */
  protected void
  testDefaultForAlgorithm(String algorithm, String expectedHash,
                          String expectedMgf, String expectedMgfHash,
                          int expectedSaltLength, int expectedTrailerField)
      throws Exception {
    ...
    assertEquals("digestAlgorithm", expectedHash,
    ...
    assertEquals("trailerField", expectedTrailerField,
                 pssParams.getTrailerField());
    if (expectedMgf.equals("MGF1")) {
      MGF1ParameterSpec mgf1Params =
          (MGF1ParameterSpec)pssParams.getMGFParameters();
      assertEquals("mgf1 digestAlgorithm", expectedMgfHash,
                   mgf1Params.getDigestAlgorithm());
    }
  }
   ...
   *
   * <p>RSA-PSS has a number of parameters. RFC 8017 specifies the parameters as
   * follows:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>The algorithm name for RSA-PSS used in jdk11 is "RSASSA-PSS".
   * Previously, the algorithm names for RSA-PSS were defined in the section
   * "Signature Algorithms" of
   * https://docs.oracle.com/javase/8/docs/technotes/guides/security/StandardNames.html
   * I.e. the proposed standard names had the format
   * <digest>with<encryption>and<mgf>, e.g., SHA256withRSAandMGF1. This name
   * only specifies the hashAlgorithm and the mask generation algorithm, but not
   * the hash used for the mask generation algorithm, the salt length and the
   * trailerField. The missing parameters can be explicitly specified with and
   * instance of PSSParameterSpec. The test below checks that distinct providers
   * use the same default values when no PSSParameterSpec is given.
   *
   * <p>In particular, the test expects that the two hash algorithm (for message
   * hashing and mgf) are the same. It expects that the saltLength is the same
   * as the size of the message digest. It expects that the default for the
   * trailerField is 1. These expectations are based on existing
   * implementations. They differ from the ASN defaults in RFC 8017.
   *
   * <p>There is no test for defaults for the algorithm name "RSASSA-PSS".
   * "RSASSA-PSS" does not specify any parameters. Using the default values from
   * RFC 8017 (i.e. SHA-1 for both hashes) leads to potential weaknesses and
   * hence is of course a bad choice. Other defaults lead to incompatibilities
   * and hence isn't a reasonable choice either. jdk11 requires that the
   * parameters are always specified. BouncyCastle however uses the SHA-1
   * default. The behaviour in jdk11 is preferable, since it requires that an
   * implementor chooses PSSParameters explicitly, and does not default to weak
   * behaviour.
   */
  @Test
  public void testDefaults() throws Exception {
    testDefaultForAlgorithm("SHA1withRSAandMGF1", "SHA-1", "MGF1", "SHA-1", 20,
                            1);
    testDefaultForAlgorithm("SHA224withRSAandMGF1", "SHA-224", "MGF1",
                            "SHA-224", 28, 1);
    testDefaultForAlgorithm("SHA256withRSAandMGF1", "SHA-256", "MGF1",
                            "SHA-256", 32, 1);
    testDefaultForAlgorithm("SHA384withRSAandMGF1", "SHA-384", "MGF1",
                            "SHA-384", 48, 1);
    testDefaultForAlgorithm("SHA512withRSAandMGF1", "SHA-512", "MGF1",
                            "SHA-512", 64, 1);
    testDefaultForAlgorithm("SHA512/224withRSAandMGF1", "SHA-512/224", "MGF1",
                            "SHA-512/224", 28, 1);
    testDefaultForAlgorithm("SHA512/256withRSAandMGF1", "SHA-512/256", "MGF1",
                            "SHA-512/256", 32, 1);
    testDefaultForAlgorithm("SHA3-224withRSAandMGF1", "SHA3-224", "MGF1",
                            "SHA3-224", 28, 1);
    testDefaultForAlgorithm("SHA3-256withRSAandMGF1", "SHA3-256", "MGF1",
                            "SHA3-256", 32, 1);
    testDefaultForAlgorithm("SHA3-384withRSAandMGF1", "SHA3-384", "MGF1",
                            "SHA3-384", 48, 1);
    testDefaultForAlgorithm("SHA3-512withRSAandMGF1", "SHA3-512", "MGF1",
                            "SHA3-512", 64, 1);
  }
   ...
   * Oracle previously specified that algorithm names for RSA-PSS are strings
   * like "SHA256WITHRSAandMGF1". See
   * http://docs.oracle.com/javase/8/docs/technotes/guides/security/StandardNames.html
   * These algorithm names fail to specify the hash function for the MGF. A
   * cleaner solution in jdk11 is to use the algorithm name "RSASSA-PSS" and
   * specify the parameters separately. This function simply attempts to return
   * an algorithm name that works.
   *
   * @param group A json dictionary containing a field "sha" with message digest
   *     (e.g. "SHA-256")
   *              and the a field "mgf" for the mask generation function (e.g.
   * "MGF1").
   ...
   * is most convenient.
   */
  protected static PublicKey getPublicKey(JsonObject object,
                                          boolean pssParamsIncluded)
      throws Exception {
    KeyFactory kf;
  ...
  protected static PSSParameterSpec getPSSParams(JsonObject group)
      throws Exception {
    String mgf = getString(group, "mgf");
    String mgfSha = getString(group, "mgfSha");
    int saltLen = group.get("sLen").getAsInt();
    return new PSSParameterSpec(mgfSha, mgf, new MGF1ParameterSpec(mgfSha),
                                saltLen, 1);
  }
   ...
   *not fail the test. This is for example used for files with test vectors that
   *use elliptic curves that are not commonly supported.
   * @param paramsIncluded if true then the enoding of the public key contains
   *the PSS parameters. The algorithm parameters of PSS are defined in appendix
   *A.2 of RFC 8017. One option is not to include the parameters and use { OID
   *rsaEncryption PARAMETERS NULL } for the algorithm identifier. Another option
   *is to include the parameters by using { OID id-RSASSA-PSS   PARAMETERS
   *RSASSA-PSS-params } as algorithm identifier. The second option requires that
   *an RSAKey contains an AlgorithmParameterSpec. The AlgorithmParameterSpec is
   *a recent addition made in jdk11. Hence many providers are currently not
   *supporting this.
   **/
  public void testRsaPss(String filename, boolean allowSkippingKeys,
                         boolean paramsIncluded) throws Exception {
    ...
    if (!generatorVersion.equals(expectedVersion)) {
      System.out.println("Expecting test vectors with version " +
    ...
    for (JsonElement g : test.getAsJsonArray("testGroups")) {
      ...
      for (JsonElement t : group.getAsJsonArray("tests")) {
        ...
        try {
          ...
          // exceptions.
          System.out.println("Signature verification throws " + ex.toString() +
        ...
        if (!verified && result.equals("valid")) {
          ...
          System.out.println("Valid signature not verified. " + filename +
                             " tcId:" + tcid + " sig:" + sig + comment);
          errors++;
        } else if (verified && result.equals("invalid")) {
          System.out.println("Invalid signature verified. " + filename +
                             " tcId:" + tcid + " sig:" + sig);
          errors++;
        } else if (verified) {
          verifiedTests++;
    ...
    if (skippedKeys > 0 || verifiedTests == 0) {
      System.out.println("File:" + filename + " number of skipped keys:" +
                         skippedKeys + " verified signatures:" + verifiedTests);
  ...
  @Test
  public void
  testRsaPss3072Sha256() throws Exception {
