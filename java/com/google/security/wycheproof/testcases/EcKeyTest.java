...
public class EcKeyTest {
  /**
   * Encodings of public keys with invalid parameters. There are multiple places
   * where a provider can validate a public key: some parameters are typically
   * validated by the KeyFactory, more validation can be done by the
   * cryptographic primitive. Unused parameters are sometimes not validated at
   * all.
   *
   * <p>This following test vectors are public key encodings with invalid
   * parameters where we expect that KeyFactory.generatePublic recognizes the
   * problem. The documentation simply claims that an InvalidKeySpecException is
   * thrown if the given key specification is inappropriate but does not specify
   * what an appropriate key exactly is. Nonetheless we expect that the
   * following minimal validations are performed: order is a positive integer,
   * cofactor is a small positive integer. Some modifications may not be
   * detected and must be caught by the primitives using them. E.g., it is
   * expensive to verify the order of the group generated by the generator and
   * hence the key factory may not verify the correctness of this parameter.
   * Thus an implementation of ECDH must not trust an order claimed in the
   * public key.
   *
   * <p>TODO(bleichen): The encoding is defined in
   * https://tools.ietf.org/html/rfc3279 Section 2.3.5. This document defines a
   * few additional requirements and options which are not yet checked: - OID
   * for id-public-key_type must be ansi-X9.62 2 - OID for id-ecPublicKey must
   * be id-publicKeyType 1 - The intended application for the key may be
   * indicated in the key usage field (RFC 3280). - EcpkParameters can be
   * implicitlyCA (not sure how we would specify the curve in this case) - the
   * version is always 1 - the points on the curves can be either compressed or
   * uncompressed (so far all points are uncompressed) - the seed value is
   * optional (so far no test vector specifies the seed) - the cofactor is
   * optional but must be included for ECDH keys. (so far all test vectors have
   * a cofactor)
   *
   * <p>RFC 3279 also specifies curves over binary fields. Because of attacks
   * against such curves, i.e. "New algorithm for the discrete logarithm problem
   * on elliptic curves" by I.Semaev https://eprint.iacr.org/2015/310 such
   * curves should no longer be used and hence testing them has low priority.
   */
  ...
  public void testEncodedPrivateKey() throws Exception {
    ...
    assertEquals(priv.getParams().getCofactor(),
                 decoded.getParams().getCofactor());
    assertEquals(priv.getParams().getCurve(), decoded.getParams().getCurve());
    assertEquals(priv.getParams().getGenerator(),
                 decoded.getParams().getGenerator());
    assertEquals(priv.getParams().getOrder(), decoded.getParams().getOrder());
  }

  /**
   * Tests key generation for given parameters. The test can be skipped if the
   * curve is not a standard curve.
   */
  void testKeyGeneration(ECParameterSpec ecParams, boolean isStandard)
      throws Exception {
    KeyPairGenerator keyGen = KeyPairGenerator.getInstance("EC");
    KeyPair keyPair;
    try {
    ...
    assertTrue(s.bitLength() >=
               EcUtil.fieldSizeInBits(ecParams.getCurve()) - 32);
  ...
  /**
   * Checks that the default key size for ECDSA is up to date. The test uses
   * NIST SP 800-57 part1 revision 4, Table 2, page 53
   * http://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-57pt1r4.pdf
   * for the minimal key size of EC keys. Nist recommends a minimal security
   * strength of 112 bits for the time until 2030. To achieve this security
   * strength EC keys of at least 224 bits are required.
   */
  @Test
  public void testDefaultKeyGeneration() throws Exception {
    ...
    if (keySize < 224) {
      fail(
          "Expected a default key size of at least 224 bits. Size of generate key is " +
          keySize);
    }
  }

  /**
   * Tries to generate a public key with a point at infinity. Public keys with a
   * point at infinity should be rejected to prevent subgroup confinement
   * attacks.
   */
  @Test
  public void testPublicKeyAtInfinity() throws Exception {
    ...
    try {
      ECPublicKeySpec pubSpec =
          new ECPublicKeySpec(ECPoint.POINT_INFINITY, ecSpec);
      fail("Point at infinity is not a valid public key. " +
