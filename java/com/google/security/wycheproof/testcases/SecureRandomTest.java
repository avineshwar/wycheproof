 ...
 *
 * The tests here are quite limited. They only test that instances are
 * non-deterministic, when required by the API. The tests do not attempt the
 * determine whether the output is pseudorandom and whether the seeds used are
 * unpredictable.
 *
 * Any instance of SecureRandom must self-seed itself if no seed is provided:
 * hence the following code must never result in deterministic or predictable
 * behaviour: <pre>{@code
 ...
 *   ...
 * }</pre>
 * An important point is that the constructur itself does not necessarily seed
 * the SecureRandom instance and that the self-seeding is only required if
 * caller does not provide any seeds. Thus the following code snippet can lead
 * to deterministic and predictable behaviour: <pre>{@code
 ...
 *   ...
 * }</pre>
 * For example "SHA1PRNG" has the property that calling setSeed after the
 * construction gives a SecureRandom instance with output that only depends on
 * the caller provided seeds.
 *
 * Once a SecureRandom instance has been seeded, all further calls of setSeed
 * must add additional randomness. It is not acceptable setSeed overrides the
 * current seed of an instance. Hence the following code must always be
 * non-deterministic. <pre>{@code SecureRandom secureRandom =
 * SecureRandom.getInstance(ALGORITHM);
 *   // The next line forces secureRandom to self-seed.
 *   secureRandom.nextBytes(new byte[1]);
 *   // Adding and additional seed. The instance remains properly seeded and
...
public class SecureRandomTest {
  ...
  public void testSeedUninitializedInstance() throws Exception {
    ...
    for (Provider.Service service : secureRandomServices()) {
      for (int i = 0; i < samples; i++) {
        ...
        assertFalse("Repeated output from " + service.getAlgorithm(),
                    seen.contains(hex));
        seen.add(hex);
      }
    }
  ...
  /**
   * Calling setSeed directly after the initialization may result in
   * deterministic results.
   *
   * The test expects that a SecureRandom instance is either completely
   * deterministic if seeded or non-deterministic and unpredictable (though the
   * test is much too simple to give any meaningful result).
   *
   ...
   *   Seeding NativePRNGNonBlocking from SUN results in non-deterministic
   * output.
   * </pre>
   *
   * jdk9 adds a class java.security.DrbgParameter, which allows to better
   * specify the expected behaviour of SecureRandom instances. Tests with these
  ...
  public void testSetSeedAfterConstruction() throws Exception {
    ...
    for (Provider.Service service : secureRandomServices()) {
      ...
      for (int i = 0; i < samples; i++) {
        SecureRandom random =
            SecureRandom.getInstance(service.getAlgorithm(), provider);
        random.setSeed(seed);
        byte[] bytes = new byte[outputsize];
        random.nextBytes(bytes);
        String hex = TestUtil.bytesToHex(bytes);
      ...
      if (seen.size() == 1) {
        System.out.println("Seeding " + service.getAlgorithm() + " from " +
                           provider.getName() +
        ...
        // ... and if the implementation is non-determinstic, there should be no
        // repetitions.
  ...
  /**
   * Calling setSeed after use adds the seed to the current state. It must never
   * replace it.
   */
  @Test
  public void testSetSeedAfterUse() throws Exception {
    ...
    for (Provider.Service service : secureRandomServices()) {
      for (int i = 0; i < samples; i++) {
        ...
        random.setSeed(seed);
        byte[] bytes = new byte[outputsize];
        // Hence it would be an error (or an unlikely false positive) if the
        // generated bytes are already known.
        random.nextBytes(bytes);
        String hex = TestUtil.bytesToHex(bytes);
        assertFalse("Repeated output from " + service.getAlgorithm(),
                    seen.contains(hex));
