 ...
 * Testing AES-GCM
 *
 * <p>Other tests using AES-GCM are: CipherInputStreamTest.java
 * CipherOuputStreamTest.java
 */
...
public class AesGcmTest {
  ...
  public static class GcmTestVector {
    ...
    public GcmTestVector(String message, String keyMaterial, String nonce,
                         String aad, String ciphertext, String tag) {
      this.ptHex = message;
  ...
  private static final GcmTestVector[] GCM_TEST_VECTORS = {
      ...
      new GcmTestVector(
          ...
          "5b9604fe14eadba931b0ccf34843dab9", "921d2507fa8007b7bd067d34",
          "00112233445566778899aabbccddeeff",
          "49d8b9783e911913d87094d1f63cc765", "1e348ba07cca2cf04c618cb4"),
      new GcmTestVector("2035af313d1346ab00154fea78322105",
      ...
      new GcmTestVector("2035af313d1346ab00154fea78322105",
                        "aa023d0478dcb2b2312498293d9a9129", "0432bc49ac344120",
                        "aac39231129872a2", "64c36bb3b732034e3a7d04efc5197785",
                        "b7d0dd70b00d65b97cfd080ff4b819d1"),
      new GcmTestVector("02efd2e5782312827ed5d230189a2a342b277ce048462193",
      ...
      // GCM uses GHASH to compute the initial counter J0 if the nonce is not 12
      // bytes long. The counter is incremented modulo 2^32 in counter mode. The
      // following test vectors verify the behavior of an implementation for
      // initial counter values J0 close to a 2^32 limit.
  ...
  /**
   * Returns the GCM test vectors supported by the current provider. This is
   * necessary since not every provider supports all parameters sizes. For
   * example SUNJCE does not support 8 byte tags and Conscrypt only supports 12
   * byte nonces. Such restrictions are often made because AES-GCM is a
   * relatively weak algorithm and especially small parameter sizes can lead to
   * easy attacks. Avoiding such small parameter sizes should not be seen as a
   * bug in the library.
   *
   * <p>The only assumption we make here is that all test vectors with 128 bit
   * tags and nonces with at least 96 bits are supported.
   */
    ...
    for (GcmTestVector test : GCM_TEST_VECTORS) {
      if (test.nonceLengthInBits != 96 || test.tagLengthInBits != 128) {
        try {
          ...
          // It would be nice if there was a way to check this without trying to
          // encrypt.
  ...
  /**
   * Typically one should always call updateAAD before any call to update. This
   * test checks what happens if the order is reversed. The test expects that a
   * correct implementation either computes the tag correctly or throws an
   * exception.
   *
   * <p>For example, OpenJdk did compute incorrect tags in this case. The bug
   * has been fixed in
   * http://hg.openjdk.java.net/jdk8u/jdk8u/jdk/rev/89c06ca1e6cc
   *
   * <p>For example BouncyCastle computes correct tags if the calls are
   * reversed, SunJCE and OpenJdk now throw exceptions.
   */
  @Test
  ...
  /**
   * JCE has a dangerous feature: after a doFinal the cipher is typically
   * reinitialized using the previous IV. This "feature" can easily break
   * AES-GCM usages, because encrypting twice with the same key and IV leaks the
   * authentication key. Hence any reasonable implementation of AES-GCM should
   * not allow this. The expected behaviour of OpenJDK can be derived from the
   * tests in
   * jdk/test/com/sun/crypto/provider/Cipher/AES/TestGCMKeyAndIvCheck.java.
   * OpenJDK does not allow two consecutive initializations for encryption with
   * the same key and IV.
   *
   * <p>The test here is weaker than the restrictions in OpenJDK. The only
   * requirement here is that reusing a Cipher without an explicit init() is
   * caught.
   *
  ...
  public void testIvReuse() throws Exception {
    for (GcmTestVector test : getTestVectors()) {
      ...
      try {
        ...
        fail("It should not possible to reuse an IV."
             + " ct1:" + TestUtil.bytesToHex(ct1) +
             " ct2:" + TestUtil.bytesToHex(ct2));
      } catch (java.lang.IllegalStateException ex) {
        // This is expected.
      }
    }
  ...
  /**
   * Checks whether the implementation requires larger ByteBuffers than
   * necessary. This test has been added mostly for debugging. E.g., conscrypt
   * failed during decryption with ByteBuffers simply because the necessary
   * outputSize was computed incorrectly.
   */
  @Test
  public void testByteBufferSize() throws Exception {
    for (GcmTestVector test : getTestVectors()) {
      ...
      cipher.init(Cipher.ENCRYPT_MODE, test.key, test.parameters);
      int outputSize = cipher.getOutputSize(test.pt.length);
      assertEquals("plaintext size:" + test.pt.length, test.ct.length,
                   outputSize);
      // Decryption
      cipher.init(Cipher.DECRYPT_MODE, test.key, test.parameters);
      outputSize = cipher.getOutputSize(test.ct.length);
      assertEquals("ciphertext size:" + test.ct.length, test.pt.length,
                   outputSize);
    }
  }
  ...
  @Test
  public void
  testLargeArrayAlias() throws Exception {
    byte[] ptVector = new byte[8192];

    // this offset is relative to the start of the input, not the start of the
    // buffer.
    for (int outputOffset = -32; outputOffset <= 32; outputOffset++) {
      ...
      for (int useUpdate = 0; useUpdate <= 1; useUpdate++) {
        ...
        byte[] inBuf = new byte[sliceLength + Math.max(inputOffsetInBuffer,
                                                       outputOffsetInBuffer)];
        byte[] outBuf = inBuf;

        System.arraycopy(ptVector, 0, inBuf, inputOffsetInBuffer,
                         ptVector.length);
        ...
        try {
          ...
          if (useUpdate > 0) {
            ctLength +=
                cipher.update(inBuf, inputOffsetInBuffer, ptVector.length,
                              outBuf, outputOffsetInBuffer);
            ctLength += cipher.doFinal(inBuf, 0, 0, outBuf,
                                       outputOffsetInBuffer + ctLength);
          } else {
            ctLength +=
                cipher.doFinal(inBuf, inputOffsetInBuffer, ptVector.length,
                               outBuf, outputOffsetInBuffer);
          }

          System.arraycopy(outBuf, outputOffsetInBuffer, inBuf,
                           inputOffsetInBuffer, ctLength);
          ...
          if (useUpdate > 0) {
            resultPtLength +=
                cipher.update(inBuf, inputOffsetInBuffer, ctLength, outBuf,
                              outputOffsetInBuffer);
            resultPtLength += cipher.doFinal(
                inBuf, 0, 0, outBuf, outputOffsetInBuffer + resultPtLength);
          } else {
            resultPtLength +=
                cipher.doFinal(inBuf, inputOffsetInBuffer, ctLength, outBuf,
                               outputOffsetInBuffer);
          }
          ...
          assertArrayEquals(
              ptVector,
              Arrays.copyOfRange(outBuf, outputOffsetInBuffer,
                                 outputOffsetInBuffer + resultPtLength));
        } catch (Throwable t) {
          throw new AssertionError(
              "testLargeByteBufferAlias failed with outputOffset=" +
                  outputOffset,
              t);
        }
      }
  ...
  /**
   * Encryption with ByteBuffers should be copy-safe even if the buffers have
   * different starting offsets and/or do not make the backing array visible.
   *
   * <p>Note that bugs in this often require a sizeable input to reproduce; the
   * default implementation of engineUpdate(ByteBuffer, ByteBuffer) copies
   * through 4KB bounce buffers, so we need to use something larger to see any
   * problems - 8KB is what we use here.
   *
  ...
  @Test
  public void
  testByteBufferShiftedAlias() throws Exception {
    byte[] ptVector = new byte[8192];
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (int outputOffset = -1; outputOffset <= 1; outputOffset++) {
        ...
        // We'll try three scenarios: Ordinary array backed buffers, array
        // backed buffers where one is read-only, and direct byte buffers.
        String mode;
        ...
        int sliceLength = cipher.getOutputSize(ptVector.length);
        int bufferSize =
            sliceLength + Math.max(inputOffsetInBuffer, outputOffsetInBuffer);
        switch (i) {
        case 0:
        case 1: {
          byte[] buffer = new byte[bufferSize];
          // It's important to slice() here as otherwise later when we flip()
          // position will be reset to 0.
          output = ByteBuffer.wrap(buffer, outputOffsetInBuffer, sliceLength)
                       .slice();
          input =
              ByteBuffer.wrap(buffer, inputOffsetInBuffer, sliceLength).slice();
        ...
        // Now that we have our overlapping 'input' and 'output' buffers, we can
        // write our plaintext into the input buffer.
        input.put(ptVector);
        input.flip();
        // Make sure the RO input buffer has the same limit in case the
        // plaintext is shorter than sliceLength (which it generally will be for
        // anything other than ECB or CTR mode)
        ...
        try {
          ...
          throw new AssertionError(
              "Overlapping buffers test failed with buffer type: " + mode +
                  " and output offset " + outputOffset,
              t);
        }
      }
    }
  ...
  /**
   * If a ByteBuffer is backed by an array and not readonly, then it is possible
   * to access the data through the .array() method. An implementation using
   * this possiblity must ensure that it considers the offset.
   */
  @Test
  ...
  /**
   * Test encryption when update and doFinal are done with empty ByteBuffers.
   * Conscrypt ignored calls to doFinal() when the ByteBuffer was empty.
   */
  @Test
  ...
  /**
   * The default authentication tag size should be 128-bit by default for the
   * following reasons: <br> (1) Security: Ferguson, N., Authentication
   * Weaknesses in GCM, Natl. Inst. Stand. Technol. [Web page],
   * http://www.csrc.nist.gov/groups/ST/toolkit/BCM/documents/comments/
   * CWC-GCM/Ferguson2.pdf, May 20, 2005. This paper points out that a n-bit tag
   * has lower strength than expected. <br> (2) Compatibility: Assume an
   * implementer tests some code using one provider than switches to another
   * provider. Such a switch should ideally not lower the security. <br>
  ...
  public void testDefaultTagSizeAlgorithmParameterGenerator() throws Exception {
    ...
    try {
      ...
      System.out.println("testDefaultTagSizeAlgorithmParameterGenerator:" +
                         ex.toString());
      return;
    }
    AlgorithmParameters param =
        AlgorithmParameterGenerator.getInstance("GCM").generateParameters();
  ...
  /**
   * Test AES-GCM wrapped around counter bug which leaks plaintext and
   * authentication key. Let's consider 12-byte IV, counter = IV || 0^31 || 1.
   * For each encryption block, the last 4 bytes of the counter is increased
   * by 1. After 2^32 blocks, the counter will be wrapped around causing counter
   * collision and hence, leaking plaintext and authentication key as explained
   * below. The library must make a check to make sure that the plaintext's
   * length never exceeds 2^32 - 2 blocks. Note that this is different from
   * usual IV collisions because it happens even if users use different IVs.
   * <br> We have: <br> J0 = IV || 0^31 || 1 <br> Plaintext: P[0], P[1], P[2],
   * .... <br> Ciphertext: <br> C[0] = Enc(K, (J0 + 1) % 2^32) XOR P[0] <br>
   ...
   * It means that after 2^32 blocks, the counter is wrapped around causing
   * counter collisions. In counter mode, once the counter is collided then it's
   * reasonable to assume that the plaintext is leaked. As the ciphertext is
   * already known to attacker, Enc(K, J0) is leaked. <br> Now, as the
   * authentication tag T is computed as GHASH(H, {}, C) XOR E(K, J0), the
   * attacker can learn GHASH(H, {}, C}. It essentially means that the attacker
   * finds a polynomial where H is the root (see Joux attack
   * http://csrc.nist.gov/groups/ST/toolkit/BCM/documents/Joux_comments.pdf).
   * Solving polynomial equation in GF(2^128) is enough to extract the
   * authentication key.
   *
   * <p>BouncyCastle used to have this bug (CVE-2015-6644).
   *
   * <p>OpenJDK8 used to have this bug
   * (http://hg.openjdk.java.net/jdk8u/jdk8u/jdk/rev/0c3ed12cdaf5)
   *
   * <p>The test is slow as we have to encrypt 2^32 blocks.
   */
  @ExcludedTest(
      providers = {ProviderType.CONSCRYPT, ProviderType.BOUNCY_CASTLE,
                   ProviderType.SPONGY_CASTLE},
      comment =
          "Conscrypt doesn't support streaming, would crash. BouncyCastle needs > 1h.")
  @SlowTest(providers = {ProviderType.BOUNCY_CASTLE, ProviderType.CONSCRYPT,
                         ProviderType.OPENJDK, ProviderType.SPONGY_CASTLE})
  @Test
  public void
  testWrappedAroundCounter() throws Exception {
    try {
      byte[] iv = new byte[12];
      byte[] input = new byte[16];
      byte[] key = new byte[16];
      ...
      for (long i = 0; i < 4294967296L + 2; i++) {
        ...
        assertFalse("GCM Wrapped Around Counter" + i,
                    Arrays.equals(output, output1));
      }
  ...
  /**
   * AES-GCM allows IVs of bit length 1 .. 2^64-1. See NIST SP 800 38d,
   * Section 5.2.1.1
   * http://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-38d.pdf
   *
   * <p>Disallowing IVs of length 0 is necessary for the following reason: if an
   * empty IV is used then the tag is an evaluation of a polynomial with the
   * hash subkey as the value. Since the polynomial can be derived from the
   * ciphertext it is known to an attacker. Therefore, any message encrypted
   * with an empty IV leaks the hash subkey. In particular, encrypting an empty
   * plaintext with an empty IV results in a ciphertext having a tag that is
   * equal to the hash subkey used in AES-GCM. I.e. both are the same as
  ...
  public void testEncryptEmptyPlaintextWithEmptyIv() throws Exception {
    ...
    try {
      cipher.init(Cipher.ENCRYPT_MODE, keySpec,
                  new GCMParameterSpec(16 * 8, emptyIv));
      byte[] ct = cipher.doFinal(input);
      // If the encryption above is not rejected then the hash key and the
      // ciphertext are the same. Both are d1bdd948ddc5a7f7a9250cf78229b84d.
      System.out.println("testEncryptEmptyPlaintextWithEmptyIv:");
      System.out.println("Encrypt with empty IV:" + TestUtil.bytesToHex(ct));
      System.out.println("Hash subkey          :" +
                         TestUtil.bytesToHex(hashkey));
  ...
  public void testDecryptWithEmptyIv() throws Exception {
    ...
    try {
      cipher.init(Cipher.DECRYPT_MODE, keySpec,
                  new GCMParameterSpec(16 * 8, emptyIv));
      String ciphertext =
          "2b65876c00d77facf8f3d0e5be792b129bab10b25bcb739b92d6e2eab241245ff449";
      ...
      // We shouldn't get here. If a provider releases unverified plaintext
      // additionally to accepting empty IVs then chosen ciphertext attacks
      // might be possible.
