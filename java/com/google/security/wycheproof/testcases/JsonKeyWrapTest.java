 ...
 * <ul>
 *   <li>The wrapping is deterministic. Hence wrapping can be compared against
 * known results. <li>The wrapping has an integrity check. Modified wrapped keys
 ...
 * I.e. each wrapping has to use a new unique IV. This IV has to be stored
 * additionally with the wrapped key and has to be passed as parameter to
 * unwrap.
 */
...
public class JsonKeyWrapTest {
  ...
  /**
   * <p> The test also determines whether different paddings lead to different
   * exceptions. Generally it is preferable when unwrapping keys with incorrect
   * paddings does not leak information about invalid paddings through
   * exceptions. Such information could be used for an attack. Ideally,
   ...
   * For example the algorithm KW (NIST SP 800 38f) is designed with the idea
   * that the underlying algorithm W is a strong pseudorandom permuation. This
   * would imply that the algorithm is resistant against attacks even if the
   * attacker get access to all the byte of W^-1 in the case of a failure. For
   * such primitives the exceptions thrown during the test are printed, but
   * distinct execeptions do not fail the test.
   *
   ...
   * @param wrappedAlgorithm the algorithm name for the secret key that is
   *     wrapped
   *    (e.g. "HMACSHA256"). The key wrap primitives wrap and unwrap byte
   * arrays. However, the JCE interface requires an instance of
   * java.security.Key with key material. The key wrap primitive does not depend
   * on wrappedAlgorithm and hence neither does the test. "HMACSHA256" is used
   * for this parameter in the tests below, simply because HMAC allows arbitrary
   * key sizes.
   * @param paddingAttacks determines whether the test fails if exceptions leak
   *     information
   *    about the padding.
   */
  ...
  public void testKeywrap(String filename, String algorithm,
                          String wrappedAlgorithm, boolean paddingAttacks)
      throws Exception {
    ...
    if (!generatorVersion.equals(expectedVersion)) {
      System.out.printf(
          "%s expects test vectors with version %s found version %s.",
      ...
      for (JsonElement t : group.getAsJsonArray("tests")) {
        ...
        try {
          ...
          if (result.equals("invalid")) {
            ...
            if (!eq) {
              System.out.printf(
                  "Incorrect wrapping for test case:%s wrapped butes:%s", tc,
                  TestUtil.bytesToHex(wrapped));
              errors++;
            }
          }
        ...
        try {
          ...
          System.out.printf(
              "Parameters accepted for wrapping but not unwrapping:%s", tc);
          errors++;
          continue;
        }
        try {
          Key keyspec =
              cipher.unwrap(expected, wrappedAlgorithm, Cipher.SECRET_KEY);
          ...
          // The JCE interface specifies that an incorrect wrapping should throw
          // an InvalidKeyException. IllegalArgumentException is thrown by the
          // SecretKeySpec constructor if the unwrapped key is empty. It is
          // unclear whether this is a bug in the code or just should be
          // documented.
    ...
    System.out.printf("Number of distinct exceptions of %s:%d", algorithm,
                      exceptions.size());
  ...
  @Test
  public void
  testAesRFC5649Wrap() throws Exception {
